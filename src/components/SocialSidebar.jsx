...
const SocialSidebar = () => {
    ...
    useEffect(() => {
        const triggerPulseSequence = () => {
            socialLinks.forEach((_, index) => {
                setTimeout(() => {
                    ...
                    // Animation lasts 800ms for more visibility
                    setTimeout(() => setActiveIndex(-1), 800)
                }, index * 400) // 400ms between each button
            })
        }
        ...
        // Initial trigger after 1 second for immediate feedback
        const initialTimeout = setTimeout(triggerPulseSequence, 1000)
