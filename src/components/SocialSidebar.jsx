...
const SocialSidebar = () => {
    ...
    useEffect(() => {
        const triggerPulseSequence = () => {
            socialLinks.forEach((_, index) => {
                setTimeout(() => {
                    ...
                    setTimeout(() => setActiveIndex(-1), 600)
                }, index * 250)
            })
        }
        ...
        const initialTimeout = setTimeout(triggerPulseSequence, 2000)
            ...
            {socialLinks.map((social, index) => (
                <a
                    ...
                    className={`
                        ...
                        transform 
                        ${activeIndex === index
                            ? `scale-125 rotate-12 z-50 border-white ring-4 ring-white/20 ${social.glow}`
                    ...
                    `}
                >
                    <span className={`text-lg lg:text-xl relative z-10 transition-transform ${activeIndex === index ? 'scale-110' : ''}`}>
                        {social.icon}
                    </span>
                </a>
            ))}
