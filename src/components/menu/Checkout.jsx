        ...
        <AnimatePresence>
            {isOpen && (
                <motion.div
                    ...
                    className="fixed inset-0 z-[110] bg-[#f8f9fa] dark:bg-[#0f1115] flex flex-col"
                >
                    {/* Header */}
                    <div className="px-6 pt-6 pb-4 flex items-center justify-between">
                        <motion.button
                            whileTap={{ scale: 0.9 }}
                            onClick={onClose}
                            className="w-12 h-12 flex items-center justify-center rounded-2xl bg-white dark:bg-white/5 shadow-sm text-gray-900 dark:text-white"
                        >
                        ...
                        <motion.button
                            ...
                            className="w-12 h-12 flex items-center justify-center rounded-2xl bg-white dark:bg-white/5 shadow-sm text-gray-400 hover:text-red-500 transition-colors"
                        >
                            <HiTrash size={24} />
                        </motion.button>
                    </div>
                    ...
                    {isSubmitted ? (
                        <div className="flex-1 flex flex-col items-center justify-center p-10 text-center">
                            <motion.div
                                ...
                                className="w-32 h-32 rounded-[2.5rem] flex items-center justify-center mb-8 bg-white dark:bg-white/5 shadow-xl"
                            >
                                <HiCheckCircle size={80} style={{ color: themeColor }} />
                            </motion.div>
                            <h2 className="text-3xl font-black text-gray-900 dark:text-white mb-2">{t.success}</h2>
                            <p className="text-gray-500 dark:text-gray-400 font-medium">{t.thanks}</p>
                        </div>
                    ) : (
                        <>
                            {/* Body */}
                            <div className="flex-1 px-6 overflow-y-auto no-scrollbar pb-32">
                                <h1 className="text-4xl font-black text-gray-900 dark:text-white mb-8 mt-4 tracking-tight">
                                    My<br />Cart List
                                </h1>
                                ...
                                {cartItems.length === 0 ? (
                                    ...
                                    <div className="space-y-6">
                                        {cartItems.map((item, index) => (
                                            ...
                                            >
                                                {/* Item Image */}
                                                <div className="w-24 h-24 rounded-[2rem] overflow-hidden shadow-md flex-shrink-0 bg-white">
                                                ...
                                                {/* Vertical +/- Controls */}
                                                <div className="flex flex-col items-center bg-gray-900 dark:bg-white/10 text-white rounded-2xl py-1 px-1">
                                                    <button
                                ...
                                <div className="mt-10 flex items-center justify-center">
                                    <div className="flex items-center gap-2 text-gray-400 dark:text-gray-500 font-bold text-sm bg-white dark:bg-white/5 py-3 px-6 rounded-full shadow-sm">
                            ...
                            <motion.div
                                initial={{ y: '100%' }}
                                animate={{ y: 0 }}
                                className="bg-white dark:bg-[#1a1c23] rounded-t-[3rem] px-8 pt-8 pb-10 shadow-[0_-10px_40px_rgba(0,0,0,0.05)] space-y-4"
                            >
                                <div className="flex justify-between items-center">
                                    <span className="text-gray-400 font-bold">Subtotal</span>
                                    <span className="text-gray-900 dark:text-white font-black text-lg">${subtotal.toFixed(2)}</span>
                                </div>
                                <div className="flex justify-between items-center">
                                    <span className="text-gray-400 font-bold">Est. Tax</span>
                                    <span className="text-gray-900 dark:text-white font-black text-lg">${tax.toFixed(2)}</span>
                                </div>
                                <div className="flex justify-between items-center">
                                    <span className="text-gray-400 font-bold">Delivery</span>
                                    <span className="text-green-500 font-black text-lg">Free</span>
                                </div>
                                <div className="h-px bg-dashed bg-gray-100 dark:bg-white/5 my-4 border-t-2 border-dashed border-gray-100 dark:border-white/5" />
                                <div className="flex justify-between items-center pb-4">
                                    <span className="text-gray-900 dark:text-white text-xl font-black">Total</span>
                                    <span className="text-gray-900 dark:text-white text-2xl font-black">${total.toFixed(2)}</span>
                                </div>
